        ...
        <div class="lg:flex">
            ...
            <div class="mt-6 lg:mt-0 lg:flex-1">
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                    ...
                    <div>
                        <h3 class="text-gray-700 uppercase dark:text-white">{{ __('Language') }}</h3>
                        <form id="language-form" action="{{ route('set.locale') }}" method="POST"
                            x-data="{ lang: '{{ currnetLanguage() }}' }" class="flex items-center space-x-2">
                            @csrf
                            <div class="relative inline-block w-12 align-middle">
                                <select id="select-language" name="locale" onchange="changeLanguage()"
                                    class="p-2 pr-10 border rounded">
                                    @foreach (languages() as $lang => $name)
                                        <option value="{{ $lang }}"
                                            {{ currnetLanguage() == $lang ? 'selected' : '' }}>
                                            {{ $name }}
                                        </option>
                                    @endforeach
